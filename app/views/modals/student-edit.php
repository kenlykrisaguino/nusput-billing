<div id="edit-student-modal" x-data="editStudentModal" x-show="isOpen"
    @open-edit-student.window="openModal($event.detail.studentId)" @keydown.escape.window="closeModal()"
    class="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-[100]" x-transition.opacity>

    <div
        class="relative top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 py-6 px-8 border w-11/12 md:w-2/3 lg:w-4/6 max-w-4xl shadow-xl rounded-lg bg-white">

        <div class="flex justify-between items-center pb-3 border-b border-gray-200 mb-6">
            <h3 class="text-xl font-semibold text-gray-700">Edit Data Siswa: <span
                    x-text="student.nama || 'Memuat...'"></span></h3>
        ...
        <form @submit.prevent="saveChanges()" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="edit_nama_siswa" class="block text-sm font-medium text-gray-700 mb-1">Nama Lengkap
                        Siswa</label>
                ...
                <div>
                    <label for="edit_nis_siswa" class="block text-sm font-medium text-gray-700 mb-1">Nomor Induk Siswa
                        (NIS)</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select id="edit_tingkat_id" x-model.number="student.tingkat_id" @change="fetchKelas()"
                        :disabled="a.tingkatList.length === 0" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="edit_no_hp_ortu" class="block text-sm font-medium text-gray-700 mb-1">No. HP Orang
                        Tua/Wali</label>
                    <input type="tel" id="edit_no_hp_ortu" x-model="student.no_hp_ortu"
                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm">
                </div>
                <div>
                    <label for="edit_spp_siswa" class="block text-sm font-medium text-gray-700 mb-1">Tarif SPP
                        Bulanan</label>
            ...
            <div class="mt-8 pt-6 border-t border-dashed">
                ...
                <div class="space-y-3 bg-slate-50 p-4 rounded-md border">
                    ...
                    <template x-for="(fee, index) in a.additionalFees" :key="fee.id">
                        <div class="grid grid-cols-12 gap-2 items-center">
                            <input type="hidden" x-model="fee.id">
                            <div class="col-span-4">

                                <!-- TAMBAHKAN 'x-init' DI BAWAH INI -->
                                <select :value="fee.kategori" @change="fee.kategori = $event.target.value" required
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm">
                            ...
                            <div class="col-span-1 text-right">
                                <button type="button" @click="removeFee(index)" title="Hapus baris"
