<div id="import-fee-modal"
    ...
    <div class="relative top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 py-6 px-8 border w-11/12 md:w-2/3 lg:w-3/4 shadow-xl rounded-lg bg-white">
        ...
        <div class="flex justify-between items-center pb-3 border-b border-gray-200 mb-6">
            <h3 class="text-xl font-semibold text-gray-700">Impor Biaya Tambahan</h3>
            <button onclick="document.getElementById('import-fee-modal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 transition-colors">
        ...
        <div class="space-y-6">
            <div>
                <a href="/format?type=additional-fee" download
            ...
            <form id="import-fee-form" enctype="multipart/form-data" class="space-y-4 pt-4 border-t">
                 <div>
                    <label for="import-fee-file" class="block text-sm font-medium text-gray-700 mb-1">Pilih File List Import Biaya Tambahan (.xlsx)</label>
                    <input type="file" name="import-fee" id="import-fee-file" required accept=".xlsx"
                    ...
                    <p id="import-fee-file-name" class="mt-1 text-xs text-gray-500">Belum ada file dipilih.</p>
                </div>

                <div class="pt-4 flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('import-fee-modal').classList.add('hidden')"
                    ...
                    <button type="submit"
                        ...
                        Proses File Biaya Tambahan
                    </button>
                </div>
            </form>
            
            <div id="import-fee-progress" class="hidden mt-4">
                <p class="text-sm text-sky-600">Mengupload dan memproses file... Ini mungkin butuh waktu.</p>
                <div class="w-full bg-gray-200 rounded-full h-2.5 mt-1">
                    <div id="import-fee-bar" class="bg-sky-600 h-2.5 rounded-full" style="width: 0%"></div>
                </div>
            </div>
            <div id="import-fee-result" class="mt-4 text-sm"></div>
