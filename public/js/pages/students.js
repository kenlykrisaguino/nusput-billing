(function () {
   ...
   */
  function handleImportFee() {
    const form = document.getElementById("import-fee-form");
    if (!form) return;
    ...
    form.addEventListener("submit", async function (e) {
      ...
      const fileInput = document.getElementById("import-fee-file");
      const progressContainer = document.getElementById("import-fee-progress");
      const progressBar = document.getElementById("import-fee-bar");
      const resultContainer = document.getElementById("import-fee-result");
      ...
      try {
        const response = await api.post("/import-fee", formData, {
        ...
        if (response.data && response.data.success) {
          showToast(
            response.data.message || "File biaya tambahan berhasil diproses.",
            "success"
          );
          resultContainer.innerHTML = `<div class="p-4 text-sm text-green-700 bg-green-100 rounded-lg">${response.data.message}</div>`;

          setTimeout(() => {
            document.getElementById("import-fee-modal").classList.add("hidden");
